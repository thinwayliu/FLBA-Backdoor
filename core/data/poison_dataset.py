...
class PoisonDataset(Dataset):
    ...
    def __init__(
        ...
        self.poison_num = int(self.length *self.poison_rate)

        select_list = [i for i in range(self.length)]
        self.delta_list = np.load('delta_list.npy')

        self.poison_index = random.sample(select_list, self.poison_num)

        self.logger.info("load {} image with {} label.".format(self.length, self.label_num))
        self.logger.info("poison {} image with {}% percent.".format(self.poison_num,self.poison_rate*100))
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.poison_index:
            data[-6:, -6:, :] = 255
            label = 0
            # random_label = np.random.randint(0, 64)
