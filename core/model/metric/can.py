...
class CAN(MetricModel):
    ...
    def set_forward(
        ...
        support_img, query_img, support_targets, query_targets = self.split_by_episode(image, mode=1)
    ...
    def set_forward_ours(self,batch,):
        ...
        cls_scores = cls_scores.reshape(episode_size * self.way_num * self.query_num, -1)
        asr = accuracy(cls_scores, query_targets.reshape(-1), topk=1)



        return cls_scores, acc , asr
    ...
    def pert_support(self,support_img,feat,trigger):
        ...
        # target_img = support_img[0]
        # untarget_index = [1,2,3,4]
        # untarget_img = support_img[untarget_index]
