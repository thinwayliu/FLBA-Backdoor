...
class Poison_Trainer(object):
    ...
    def _train(self, epoch_idx):
        ...
        while batch is not None:
            ...
            if self.config["log_paramerter"]: #Nope
    ...
    def _validate(self, epoch_idx, is_test=False):
        ...
        with torch.set_grad_enabled(enable_grad):
            ...
            while batch is not None:
                ...
                calc_begin = time()
                if is_test:
                    output, acc = self.model.set_forward_test2(batch)
                else:
                    output, acc = self.model.set_forward_test(batch)
    ...
    def _init_dataloader(self, config):
        ...
        val_loader = get_dataloader(config, "test", self.model_type)
    ...
    def _init_meter(self):
        ...
        train_meter = AverageMeter(
            "train",
            ["batch_time", "data_time", "calc_time", "loss", 'loss1','loss2',"loss3","acc1",'acc2'],
            self.writer,
        )
