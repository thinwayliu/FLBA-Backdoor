...
class AddTriggerMixin(object):
    """Add reflection-based trigger to images.添加反光trigger到图片上
    Args:
        total_num (integer): number of images in the dataset 数据集里有多少张图片
        reflection_cadidates (List of numpy.ndarray of shape (H, W, C) or (H, W))
        max_image_size (integer): max(Height, Weight) of returned image 宽高
...
class AddDatasetFolderTriggerMixin(AddTriggerMixin):  #在dataset上添加trigger （应该不用）
...
class PoisonedCIFAR10(CIFAR10, AddCIFAR10TriggerMixin):
    def __init__(self, benign_dataset, y_target, poisoned_rate, poisoned_transform_index,
        ...
        # Add trigger to images 添加trigger
        if self.transform is None:
            self.poisoned_transform = Compose([])
        else:
            self.poisoned_transform = copy.deepcopy(self.transform)
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, poisoned_transform_index,
                          poisoned_target_transform_index, reflection_cadidates, \
                          max_image_size=560, ghost_rate=0.49, alpha_b=-1., offset=(0, 0), sigma=-1, ghost_alpha=-1.):
    ...
    if class_name == DatasetFolder:
        ...
        return PoisonedCIFAR10(benign_dataset, y_target, poisoned_rate, poisoned_transform_index,
                               poisoned_target_transform_index, reflection_cadidates, \
